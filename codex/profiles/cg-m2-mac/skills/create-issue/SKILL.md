---
name: "create-issue"
description: "Issue を起票する。Claude command /create-issue 相当を Codex CLI で実行する。"
metadata:
  short-description: "Issue を起票する"
---

<!-- codex-profile-generated-from-prompt: prompts/create-issue.md -->

# create-issue

この skill は Claude command `/create-issue` から変換した Codex 用 command skill です。

## Codex での呼び出し

Codex CLI では `/create-issue` ではなく、`$create-issue` または `/skills` からこの skill を呼び出してください。
引数は `$create-issue` の後ろに自然文として続けます。

```text
$create-issue <arguments>
```

元 prompt 内の `$ARGUMENTS` や slash command 表記は、`$create-issue` の後ろに書かれた引数として解釈してください。
