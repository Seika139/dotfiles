...
| APM prompt primitive の Codex 対応  | (**歴史的記述**: prompt primitive は migration-plan.md §9.8 で全廃) ❌ prompt は Codex に出力されない ("Codex has no prompts/commands primitive")。Codex カバーには skill primitive 必須                                                                                                                                                  |
...
| user-scope primitive 対応マトリクス | (prompts に関する項は §9.8 の prompt 全廃後は無関係) **fully**: claude, gemini, agent-skills, copilot-cowork。**partial**: copilot, cursor, opencode, codex, windsurf。**特定非対応**: copilot (prompts, instructions), cursor (instructions), opencode (hooks), windsurf (instructions)                                   |
...
- skill frontmatter の**必須 2 キー**: `name` (小文字英数ハイフン・1–64字・dir 名と一致必須)、`description` (命令形 "Use when …"・≤1024字)。
- **skill frontmatter は上記 2 キーに限られない** (旧記述はこの 2 キーのみを挙げており、「skill は 2 キーしか持てない」という誤解の出典になっていた)。`allowed-tools` / `argument-hint` / `model` / `context` / `disable-model-invocation` / `user-invocable` 等も有効で、公式プラグイン `example-plugin/skills/example-command/SKILL.md` が `allowed-tools` と `argument-hint` を実際に使っている。`user-invocable` の既定値は true なので skill は `/` メニューにも出る。
- **`allowed-tools` は制限ではなく追加的な事前承認**である。実装は `alwaysAllowRules.command` に追記するだけで、利用可能ツール集合を絞る経路は無い。例外は `context: fork` の skill を subagent 起動する経路のみで、そこだけ置換 (絞り込み) になる。
- ~~prompt 命名は `<name>.prompt.md` (`.prompt.md` suffix 必須。無いと無視される)。~~ → **prompt primitive は migration-plan.md §9.8 で全廃**。以下は当時の記述。`.claude/commands/` 形式は Claude Code 内部で legacy 扱い (`loadedFrom` が `"commands_DEPRECATED"`) であり、SKILL.md と同一のパーサを通る。
