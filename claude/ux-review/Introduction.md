...
```text
1. 初回セットアップ（1 度だけ）
   └─ Playwright MCP をユーザーグローバルに登録 → Claude Code 再起動
...
スラッシュコマンド本体は `~/.claude/commands/ux-review.md`
（実体は `~/dotfiles/claude/profiles/cg-m2-mac/commands/ux-review.md`）。
...
Claude Code を **一度終了して再起動**（MCP を読み込むため）。
...
内部で `cp -R ~/dotfiles/claude/ux-review/templates/. <target-path>/`
...
```bash
mkdir -p <target-path>
cp -R ~/dotfiles/claude/ux-review/templates/. <target-path>/
```
...
`~/.claude/commands/ux-review.md` に以下が埋め込まれている:

- MCP 未接続時は `claude mcp add ...` を案内して停止
...
雛形を改善したくなったら `~/dotfiles/claude/ux-review/templates/` を直接編集する。
