...
description: "git worktree で隔離された作業環境を作成し、複数セッションの競合を防ぐ"
---

# Worktree — セッション隔離コマンド

同じリポジトリを複数の Claude セッションが同時に編集する場合に、git worktree を使って作業ディレクトリを分離し、変更の競合を防止します。
...
- 引数なし → 新しい worktree を作成してセッションを移動する
- `worktree 名` → 指定した名前で worktree を作成する（例: `/worktree fix-auth-bug`）
- `--exit` → 現在の worktree を保持したまま元のディレクトリに戻る
- `--exit-remove` → 現在の worktree を削除して元のディレクトリに戻る
...
`EnterWorktree` ツールを使用して worktree を作成する。
...
このセッションは隔離された worktree 内で作業します。
他の Claude セッションと競合する心配はありません。

作業が終わったら:
- /worktree --exit         → worktree を保持して戻る
- /worktree --exit-remove  → worktree を削除して戻る
```
...
- 各セッションで `/worktree` を実行してから作業を開始する
