...
class Region:
    ...
    def subregion(self, left: float, top: float, right: float, bottom: float) -> Region:
        if not (
            0.0 <= left <= 1.0
            and 0.0 <= top <= 1.0
            and 0.0 <= right <= 1.0
            ...
            raise ValueError("サブ領域の相対座標は 0.0〜1.0 の範囲で指定してください")
        if right <= left or bottom <= top:
            raise ValueError("サブ領域は幅と高さが必要です")
        width = self.width
        height = self.height
        sub_left = self.left + left * width
        sub_top = self.top + top * height
        sub_right = self.left + right * width
        sub_bottom = self.top + bottom * height
        return Region(left=sub_left, top=sub_top, right=sub_right, bottom=sub_bottom)
